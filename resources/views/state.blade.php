...
@section('page_title', 'Sales State')

@section('content')
<div class="container mt-4">
 
      <button class="btn button" data-bs-toggle="modal" data-bs-target="#createStateModal">
            <i class="bi bi-plus-lg"></i> Create State
        </button>
        
<div class="table-responsive">
<table class="table table-hover table-bordered align-middle text-center border shadow-sm rounded" id="salesStateTable">
    ...
    <form id="createStateForm">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="createStateModalLabel">Create State</h5>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control" id="edit_state_name" required>
          </div>
        </div>
        <div class="modal-footer">
...
function loadState(page = 1) {
    $.get(`{{ route('state.fetch') }}?page=${page}`, function (data) {
        let rows = '';
        $.each(data.data, function (i, s) {
            rows += `<tr>
                <td>${s.state_name}</td>
                <td>
                    <button class="btn btn-sm btn-primary editBtn" data-id="${s.id}" data-name="${s.state_name}">
